...
"""
    All_Aux_Ham(na::Int64, r, Aux_Sym_Zones::Bool, Real_Fourier::Bool, tol::Float64)
    This function generates the Pauli strings and coefficients for all the off diagonal auxiliary matrices.
"""
function All_Aux_Ham(na::Int64, r, Aux_Sym_Zones::Bool, Real_Fourier::Bool, tol::Float64)
    ...
    if Real_Fourier
        for j in 1:length(rVec)
            ...
            Floq_Aux_Coeff_arr[j] = (2.0) .* Floq_Aux_Coeff_arr[j] # Multiply by 2.0 due to deletion of imaginary part and 0.5 in the coefficients of real part. 
        end
    end 
...
function Floq_xyz_model(L::Int64,na::Int64,r,Omg::Float64,Jx,Jy,Jz,dJx,dJy,dJz,Aux_Sym_Zones::Bool,Real_Fourier::Bool,tol::Float64, PBCs)
    ...
    Str_Coeff_Floq_Aux= Aux_Diag_Str_Coeff(na,Omg) ## I guess here we have 
    ...
    Str_Coeff_OffDiag=All_Aux_Ham(na,r,Aux_Sym_Zones,Real_Fourier,tol)
    if Real_Fourier
        for j in 1:length(r)
        ...
        for j in 1:length(r)
            for FloqJ in 1:length(Str_Coeff_OffDiag[1][j])
                for site=1:L
                    ...
                    push!(Floq_Mag_Aux_Str_arr,Bx_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*dBx[j])
                    push!(Floq_Mag_Aux_Str_arr,By_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*dBy[j])
                    push!(Floq_Mag_Aux_Str_arr,Bz_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*dBz[j])
                end
            end  
        end        
    else
        for j in 1:length(r)
            for FloqJ in 1:length(Str_Coeff_OffDiag[1][j])
                for site=1:L
                    ...
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*dBx[j])
                    push!(Floq_Mag_Aux_Str_arr,Bx_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*conj(dBx[j]))
                    push!(Floq_Mag_Aux_Str_arr,By_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*dBy[j])
                    push!(Floq_Mag_Aux_Str_arr,By_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*conj(dBy[j]))
                    push!(Floq_Mag_Aux_Str_arr,Bz_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*dBz[j])
                    push!(Floq_Mag_Aux_Str_arr,Bz_str)
                    push!(Floq_Mag_Aux_Coeff_arr, Str_Coeff_OffDiag[2][j][FloqJ]*0.5*conj(dBz[j]))
                end
            end  
        end 
        # removing strings with zero weight
